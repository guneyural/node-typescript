...
function getPath({
  ...
  extension: VideoDocument["extension"];
}) {
...
export async function uploadVideoHandler(req: Request, res: Response) {
  ...
  bb.on("file", async (_, file, info) => {
    ...
    const extension = <VideoDocument["extension"]>info.mimeType.split("/")[1];
...
export async function updateVideoHandler(
  ...
  if (video.owner !== userId) return res.status(401).send("Unathorized");
